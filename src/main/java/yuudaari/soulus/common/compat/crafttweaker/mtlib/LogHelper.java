package yuudaari.soulus.common.compat.crafttweaker.mtlib;
...
public class LogHelper {
	...
	public static void logPrinted (IPlayer player) {
		...
		if (player != null) {
			player.sendChat(CrafttweakerImplementationAPI.platform
	...
	public static String getStackDescription (Object object) {
		...
		} else if (object instanceof Ingredient && !((Ingredient) object)
			.apply(ItemStack.EMPTY) && ((Ingredient) object).getMatchingStacks().length > 0) {
	...
	public static String getCraftingDescription (ShapedOreRecipe recipe) {
		...
		Ingredient[][] recipes = InputHelper.getMultiDimensionalArray(Ingredient.class, recipe.getIngredients()
			.toArray(new Ingredient[0]), height, width);
	...
	public static String getCraftingDescription (ShapedRecipes recipe) {
		...
		Ingredient[][] recipes = InputHelper.getMultiDimensionalArray(Ingredient.class, recipe.getIngredients()
			.toArray(new Ingredient[0]), recipe.recipeHeight, recipe.recipeWidth);
		return getListDescription(Arrays.asList(recipes));
	}
}
